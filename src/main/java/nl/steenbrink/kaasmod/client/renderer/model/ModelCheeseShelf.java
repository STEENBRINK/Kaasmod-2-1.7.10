...
public class ModelCheeseShelf extends ModelBase
{
    //fields
    private final RenderItem customRenderItem;
    ModelRenderer Bottom;

    public ModelCheeseShelf(){
    ...
    public void renderBlock(TileEntityCheeseShelf tileEntityCheeseShelf, double x, double y, double z) {
        ...
        if (tileEntityCheeseShelf.isCrafting())
            speedMultiplier = 4f;
        float rotationAngle = (float) (720.0 * speedMultiplier * (System.currentTimeMillis() & 0x3FFFL) / 0x3FFFL);

        EntityItem ghostEntityItem = new EntityItem(tileEntityCheeseShelf.getWorldObj());
        ghostEntityItem.hoverStart = 0.0F;
        ghostEntityItem.setEntityItemStack(tileEntityCheeseShelf.getStackInSlot(0));
